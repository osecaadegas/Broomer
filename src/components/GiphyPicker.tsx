...
export function GiphyPicker({ onSelect, onUpload, onClose }: Readonly<Props>) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (deferredQuery) url.searchParams.set("q", deferredQuery.slice(0, 50));

        const response = await fetch(url, { signal: controller.signal });
        const data = (await response.json()) as GiphyResponse;
        if (!response.ok) throw new Error(data.meta?.msg || "Could not load GIFs");
        setItems(data.data ?? []);
      } catch (loadError) {
  ...
  function chooseGif(item: GiphyItem) {
    ...
    if (!url) return;
    const analyticsUrl = item.analytics?.onclick?.url;
    if (analyticsUrl) void fetch(analyticsUrl, { mode: "no-cors" });
  ...
  return (
    <div className="absolute bottom-14 left-0 right-0 z-20 overflow-hidden rounded-md border border-[#8b5b79]/45 bg-[#110b16]/98 shadow-[0_1.5rem_4rem_rgba(0,0,0,0.8)] backdrop-blur-xl">
      <div className="flex items-center gap-2 border-b border-[#8b5b79]/25 p-2.5">
        <span aria-hidden className="text-[#d8b566]">⌕</span>
        <input
          type="search"
          value={query}
      ...
      <div className="h-64 overflow-y-auto p-2">
        {!apiKey ? (
          <div className="grid h-full place-items-center px-5 text-center">
            <div>
              <p className="font-serif text-base italic text-[#e6c4d4]">GIPHY library needs its key</p>
          ...
          <p className="py-10 text-center text-xs text-stone-500">Summoning GIFs...</p>
        ) : error ? (
          <p role="alert" className="py-10 text-center text-xs text-red-300">{error}</p>
        ) : items.length === 0 ? (
          <p className="py-10 text-center text-xs text-stone-500">No GIFs found.</p>
        ) : (
          <div className="grid grid-cols-3 gap-1.5">
            {items.map((item) => {
              const preview = item.images?.fixed_width_small?.url ?? item.images?.fixed_width?.url;
