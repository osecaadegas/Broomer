...
export function MoodSelfieInterlude({ onComplete }: Readonly<Props>) {
  ...
  return createPortal(
    <dialog
      ...
      className="fixed inset-0 z-[75] m-0 flex h-dvh max-h-none w-screen max-w-none items-center justify-center overflow-hidden overscroll-none border-0 bg-[#09070d]/95 px-4 py-4 text-center backdrop-blur-xl sm:py-6"
    >
      {phase === "sentences" ? (
        <p
          key={sentenceIndex}
        ...
        <div className="flex h-full min-h-0 w-full max-w-md flex-col items-center justify-center gap-4 sm:gap-5">
          <div className="animate-selfie-reveal relative aspect-[3/4] h-[min(52dvh,calc((100vw-2rem)*4/3))] max-h-[calc(100dvh-11rem)] w-auto max-w-full shrink overflow-hidden rounded-2xl border border-[#c46b91]/45 bg-[#160f19] shadow-[0_1.5rem_4rem_rgba(0,0,0,0.65),0_0_3rem_rgba(180,56,112,0.18)]">
          ...
          {cameraAccepted && phase !== "unavailable" && (
            <p className="animate-hope-message w-full max-w-md shrink-0 px-1 font-serif text-[clamp(0.95rem,4vw,1.25rem)] leading-relaxed text-[#efadc8]">
              {finalMessage}
            </p>
          )}
        </div>
